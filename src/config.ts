// Database
export const steamJSONPath =
  "C:\\bots\\LBF-Link\\build\\src\\databases\\steam.json";

// Save files
export const saveFilesPath = "C:\\theisleserver\\TheIsle\\Saved\\PlayerData";

// Discord Channels
export const discordGuildID = "1084674025961291786";
export const discordMapChannelID = "1406611762790404220";
export const discordOptOutLiveStats = "1407116264173862923";
export const discordActiveMembersChannelID = "1406612006571606027";
export const discordDinoPopulationChannelID = "1406682346002055198";
export const discordAdminBunkerLiveMembersChannelID = "1407456598191767553";

// Discord Messages
export const discordOptOutMessageID = "1407418656576770068";

// Discord Roles
export const liveStatsOptOutRoleID = "1407415543660806178";

// Notify Gnome
export const discordOptOutNotifyUserID = "363120478791008257";

// Save editor
export const saveEditorExePath = "C:\\TheIsleSaveEditor\\islesave.exe";
export const saveEditorSavesPath = "C:\\TheIsleSaveEditor\\saves";

// emojis
export const thumbsUpEmoji = "👍";
export const noEntryEmoji = "⛔";
export const redCircleEmoji = "🔴";
export const greenCircleEmoji = "🟢";

// Dinos
export const dinoTypes: Record<string, "herbivore" | "carnivore" | "omnivore"> =
  {
    Diabloceratops: "herbivore",
    Dryosaurus: "herbivore",
    Hypsilophodon: "herbivore",
    Pachycephalosaurus: "herbivore",
    Stegosaurus: "herbivore",
    Tenontosaurus: "herbivore",
    Maiasaura: "herbivore",
    Triceratops: "herbivore",
    Carnotaurus: "carnivore",
    Ceratosaurus: "carnivore",
    Deinosuchus: "carnivore",
    Dilophosaurus: "carnivore",
    Herrerasaurus: "carnivore",
    Omniraptor: "carnivore",
    Pteranodon: "carnivore",
    Troodon: "carnivore",
    Tyrannosaurus: "carnivore",
    Allosaurus: "carnivore",
    Beipiaosaurus: "omnivore",
    Gallimimus: "omnivore",
  };

// Admins
export const adminsSteamIDs: string[] = [
  "76561198447466615",
  "76561198125820975",
  "76561198963322906",
  "76561199105516477",
  "76561198128393754",
  "76561198052472101",
  "76561199078077631",
  "76561199357757644",
  "76561198201569029",
  "76561198204245116",
  "76561198127024374",
  "76561198354338999",
  "76561199238360631",
  "76561198104363318",
];

// Admins discord ids
export const adminDiscordIDs = [
  "363120478791008257",
  "313906529709457408",
  "574645488146579467",
  "692950449367678976",
  "400068493250134016",
  "726852650703912980",
  "740446014397218867",
  "361615858683871235",
  "526468670810226723",
  "239118884265787393",
  "801144390516473896",
  "1129629821660569721",
  "1134930363974701156",
  "191201062663880704",
];
